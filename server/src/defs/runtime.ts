...
export type SecretKey =
  ...
  | "ADMIN_TOKEN"
  // Aliyun DashScope (百炼) API key. Shared by:
  //   #07 voice input — Paraformer-v2 async transcription (/api/asr)
  //   #06 image input — Qwen-VL-Max chat-with-image (/api/chat image branch)
  // Both endpoints share this single key. Bearer-token auth, no signing
  // complexity. Set via `edgespark secret set DASHSCOPE_API_KEY`.
  | "DASHSCOPE_API_KEY";
