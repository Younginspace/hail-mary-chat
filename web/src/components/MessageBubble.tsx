...
export default function MessageBubble({
  ...
  if (!multi) {
    return (
      <div ref={bubbleRef}>
        {renderBlock(blocks[0].speaker, blocks[0].rawContent, 0, true)}
      </div>
    );
  }

  // Multi-speaker path: wrap the whole message in a neutral container
  // so GSAP mount animation + share-select mode + refs still target
  // one element, then render each block as its own visual bubble.
  return (
    <div ref={bubbleRef} className="message-multi">
      {blocks.map((b, i) =>
        renderBlock(b.speaker, b.rawContent, i, i === blocks.length - 1),
      )}
    </div>
  );
}
